...
void main(void)
{
    GPIO   = 0x00;          // Configuracion de todos los pines en bajo inicialmente
    TRISIO = 0b00001000;    // Configuracion de GP3 como entrada y los demás como salidas
    ANSEL  = 0x00;          // Configuracion de los pines como digitales
    ...
    while (1)
    {
        ...
        if (boton != estado_anterior)
        {
            if (boton == 0) // Cuando el botón cambia de alto a bajo (presionado)
            {
                ...
                // Espera un breve instante para que puedas ver el cambio 
